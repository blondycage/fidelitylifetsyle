...
export async function PUT(request: NextRequest) {
  try {
    ...
    console.log('🔄 Updating event product:', body);

    const backendUrl = `${PRODUCT_BASE_URL}/update/event`;
    ...
    return await handleBackendResponse(backendResponse, {
      ...
      fallbackMessage: 'Failed to update event product'
    });
  } catch (error) {
    console.error('❌ Update Event Product API Error:', error);
