...
export async function PUT(request: NextRequest) {
  try {
    ...
    console.log('🔄 Updating accommodation product:', body);

    const backendUrl = `${PRODUCT_BASE_URL}/update/accomodation`;
    ...
    return await handleBackendResponse(backendResponse, {
      ...
      fallbackMessage: 'Failed to update accommodation product'
    });
  } catch (error) {
    console.error('❌ Update Accommodation Product API Error:', error);
