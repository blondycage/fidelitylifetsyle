...
export async function PUT(request: NextRequest) {
  try {
    ...
    console.log('🔄 Updating car product:', body);

    const backendUrl = `${PRODUCT_BASE_URL}/update/car`;
    ...
    return await handleBackendResponse(backendResponse, {
      ...
      fallbackMessage: 'Failed to update car product'
    });
  } catch (error) {
    console.error('❌ Update Car Product API Error:', error);
