...
export async function PUT(request: NextRequest) {
  try {
    ...
    console.log('🔄 Updating food product:', body);

    const backendUrl = `${PRODUCT_BASE_URL}/update/food`;
    ...
    return await handleBackendResponse(backendResponse, {
      ...
      fallbackMessage: 'Failed to update food product'
    });
  } catch (error) {
    console.error('❌ Update Food Product API Error:', error);
