...
const ROOM_CREATE_URL = 'http://45.33.68.176:9091/api/v1/product/create/room';

export async function POST(request: NextRequest) {
  try {
    const body = await request.json();
    
    console.log('Creating room with payload:', body);
    
    const backendResponse = await fetch(ROOM_CREATE_URL, {
      method: 'POST',
      headers: {
        'accept': '*/*',
    ...
    return NextResponse.json(
      { responseCode: 500, responseMessage: 'Failed to create room' },
      { status: 500 }
    );
  }
}
