...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('🚀 Event Create API Request:', {
      ...
      method: 'POST',
      url: 'http://45.33.68.176:9091/api/v1/product/create/event',
      payload: body
    });
    ...
    const backendResponse = await fetch('http://45.33.68.176:9091/api/v1/product/create/event', {
      method: 'POST',
      headers: {
    ...
    console.log('✅ Event Create Backend Response:', {
      timestamp: new Date().toISOString(),
      status: backendResponse.status,
      data: data
    });
    ...
    console.error('❌ Event Create API Error:', error);
    return NextResponse.json(
      {
        responseCode: 500,
